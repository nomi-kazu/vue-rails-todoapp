    ...
    <div v-bind:class="{ 'is-active': modalActive }" class="modal">
      ...
      <div class="modal-content">
        <div class="field is-grouped mt-6">
          ...
          <p class="control">
            <a @click="updateTodo(editTodo.id)" class="button is-link">
              update
            </a>
          </p>
        </div>
...
export default {
  ...
  mounted() {
    axios
      .get('https://vue-rails-backend-api.herokuapp.com/v1/todos')
      .then(response => {
  ...
  methods: {
    createTodo: function() {
      axios
        .post('https://vue-rails-backend-api.herokuapp.com/v1/todos', this.todo)
        .then(response => {
          this.updateTodos();
          this.todo.title = '';
        })
    ...
    destroyTodo: function(id) {
      axios
        .delete(`https://vue-rails-backend-api.herokuapp.com/v1/todos/${id}`)
        .then(response => {
          this.updateTodos();
        })
    },
    updateTodo: function(id) {
      axios
        .patch(`https://vue-rails-backend-api.herokuapp.com/v1/todos/${id}`, this.editTodo)
        .then(response => {
